...
public class CartService {
    ...
    public Future<String> executor(){
        ...
        return AsyncResult.forValue("hello executor");
    }
    ...
    @Async
    public ListenableFuture<String> executor(){
        try {
            System.out.println("这是一个executor1方法,开始执行");
              TimeUnit.SECONDS.sleep(5);
            System.out.println("这是一个executor1方法，结束执行");
        ...
        if(!CollectionUtils.isEmpty(unloginCarts)){
            unloginCarts.forEach(cart -> {
                ...
                if(loginHashOps.hasKey(skuId)){
                    ...
                    //如果登录状态的购物车不包含该商品，增新增一条记录
                    //将之前的userkey更新为userId
    ...
    public void updateNum(Cart cart) {
        ...
        //查询redis中的购物车记录
        String json = hashOps.get(cart.getSkuId()).toString();
        cart = JSON.parseObject(json, Cart.class);
        cart.setCount(count);//更新购物车中商品数量
    ...
    public List<Cart> queryCheckedCartsByUserId(Long userId) {
        ...
        return cartJsons.stream()
                .map(cartJson->JSON.parseObject(cartJsons.toString(), Cart.class))
